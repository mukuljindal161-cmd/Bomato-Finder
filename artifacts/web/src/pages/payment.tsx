...
type OrderData = { restaurantName: string; restaurantId: number; items: OrderItem[] };
type PaymentMethod = "upi" | "card" | "cod";

const paymentOptions: { value: PaymentMethod; label: string; description: string; icon: React.ReactNode }[] = [
  { value: "upi", label: "UPI", description: "Pay instantly via any UPI app", icon: <Smartphone className="w-5 h-5" /> },
  { value: "card", label: "Credit / Debit Card", description: "Visa, Mastercard, RuPay and more", icon: <CreditCard className="w-5 h-5" /> },
  { value: "cod", label: "Cash on Delivery", description: "Pay when your order arrives", icon: <Banknote className="w-5 h-5" /> },
];
...
export default function PaymentPage() {
  ...
  const grandTotal = useMemo(() => orderData?.items.reduce((s, i) => s + i.price * i.quantity, 0) ?? 0, [orderData]);
  const totalItems = useMemo(() => orderData?.items.reduce((s, i) => s + i.quantity, 0) ?? 0, [orderData]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await apiFetch<{ order: { id: string; orderNumber: string; total: string; status: string; createdAt: string; paymentMethod: string } }>(
        "/orders",
        {
          method: "POST",
          auth: true,
  ...
  return (
    <div className="min-h-screen bg-background pb-36">
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-6 space-y-5">
        {!user && (
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-700 mt-0.5">You need an account to complete checkout.</p>
              <Link href={`/login`}>
                <button className="mt-2 text-xs font-bold text-amber-800 underline">Sign In →</button>
              </Link>
            </div>
          </div>
        )}
        ...
        {apiError && (
          <div className="bg-destructive/10 border border-destructive/20 rounded-2xl p-4 text-sm font-medium text-destructive">
            {apiError}
          </div>
        )}
          ...
          <div className="px-4 pt-2 pb-3 space-y-2">
            {orderData.items.map((item) => (
              <div key={item.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                ...
                <span className="text-sm font-bold text-foreground ml-4 shrink-0">${(item.price * item.quantity).toFixed(2)}</span>
              </div>
            ))}
          </div>
          <div className="px-4 py-3 border-t border-border bg-muted/30 flex items-center justify-between">
            <span className="text-sm font-semibold text-muted-foreground">{totalItems} {totalItems === 1 ? "item" : "items"} · {orderData.restaurantName}</span>
        ...
        <section className="bg-card border border-card-border rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="p-3 space-y-2">
            {paymentOptions.map((option) => {
              ...
              return (
                <label key={option.value} className={`flex items-center gap-4 p-3.5 rounded-xl cursor-pointer border transition-all ${selected ? "border-primary bg-primary/5" : "border-border hover:border-muted-foreground/40 hover:bg-muted/30"}`}>
                  <input type="radio" name="paymentMethod" value={option.value} checked={selected} onChange={() => setPaymentMethod(option.value)} className="sr-only" />
                  <div className={`shrink-0 ${selected ? "text-primary" : "text-muted-foreground"}`}>{option.icon}</div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-semibold ${selected ? "text-primary" : "text-foreground"}`}>{option.label}</p>
                    <p className="text-xs text-muted-foreground">{option.description}</p>
                  </div>
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${selected ? "border-primary" : "border-border"}`}>
        ...
        <div className="max-w-2xl mx-auto space-y-2">
          ...
          >
            {loading ? (
              ...
              <>{isCOD ? "Place Order" : "Pay Now"} · ${grandTotal.toFixed(2)}</>
            )}
          </button>
